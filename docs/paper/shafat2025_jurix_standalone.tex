...
%%
%% ACM sigconf double-column format.
%%
...
%%   3. Add CCS concepts, DOI, and ISBN in camera-ready.
%% ============================================================
\documentclass[sigconf,screen]{acmart}

%% ── Packages not bundled with acmart ─────────────────────────
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{graphicx}
...
\acmConference[JURIX 2025]{38th International Conference on Legal
  Knowledge and Information Systems}{December 2025}{Amsterdam, The Netherlands}
\acmYear{2025}
\copyrightyear{2025}
...
\title{Hybrid Sparse-Dense Retrieval for EU Climate Law:
  Evaluating BM25, nomic-embed-text-v1.5, and Reciprocal Rank Fusion
...
\email{rameezshafat.wani2@mail.dcu.ie}

\author{Ananya Warior}
\affiliation{%
  \institution{School of Computing, Dublin City University}
  \city{Dublin}
  \country{Ireland}
}
\email{ananyasalil.warior2@mail.dcu.ie}

%% ── Keywords ─────────────────────────────────────────────────
\keywords{EU Climate Law; legal information retrieval; hybrid
  retrieval; BM25; nomic-embed; reciprocal rank fusion;
...
\BlankLine
\tcp{Step 1 --- concurrent retrieval (ThreadPoolExecutor)}
$\mathcal{R}_s,\;\mathcal{R}_d \leftarrow
  ...
  \bigr\}$\;
\BlankLine
\tcp{Step 2 --- weighted RRF fusion (Eq.~\ref{eq:rrf})}
$\mathcal{D} \leftarrow
...
\caption{Ablation: sensitivity of hybrid \rrf to the smoothing
  constant $k$ (Eq.~\ref{eq:rrf}), fixed weights $w_s{=}1$,
  $w_d{=}5$.  Bold: baseline configuration ($k{=}20$).}
\label{tab:ablation}
\centering
\small
\begin{tabular}{@{}ccc@{}}
