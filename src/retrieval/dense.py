...
class DenseRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        self._article_map: list[LegalArticle] = []  # position → article
    ...
    def index(self, articles: list[LegalArticle]) -> None:
        ...
        faiss.normalize_L2(matrix)  # in-place; now dot product = cosine sim

        self._index = faiss.IndexFlatIP(self._embed_dim)
        self._index.add(matrix)
        self._article_map = list(articles)  # copy so callers can mutate theirs
