...
manually constructed benchmark of 102~queries over EU Climate Law.
...
Evaluation uses a stratified split; all reported metrics are from the
...
Our contributions are: (i)~a 102-query manually verified benchmark for
...
The benchmark contains \textbf{102~queries} covering all eleven EU
Climate Law instrument families.
...
retrieval.
The 102~queries comprise two cohorts: 71~original plain-language
queries (split 49~validation / 22~test) and 31~queries derived from
written questions submitted by Members of the European Parliament (MEPs)
to the Commission, added to the test partition after the validation
split was fixed; the Commission's official answers independently
corroborate the CELEX relevance judgements.
The \textbf{53-query sealed test set} thus consists of the 22~original
test queries plus the 31~EP-derived queries.
...
that dense misses.
This is partly a design limitation of RRF on small statutory corpora:
RRF is rank-biased and discards score confidence entirely, so on a
1{,}166-document corpus where the dense retriever already places the
correct instrument at rank~1 for most queries, BM25's rank signal
provides no additional discriminative information~\cite{18}.
