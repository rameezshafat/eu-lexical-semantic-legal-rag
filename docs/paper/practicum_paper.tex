...
general corpora, and none controls for vocabulary leakage between
evaluation queries and the indexed statutes, which we show can
single-handedly determine which retrieval paradigm appears superior.
This paper evaluates three first-stage retrieval configurations on a
102-query benchmark over EU Climate Law: BM25 lexical search, \nomic
dense retrieval (768-dim, 8{,}192-token context), and a hybrid
combining both via weighted Reciprocal Rank Fusion (RRF, $k{=}20$,
dense weight~5).
...
weighted lexical-semantic fusion provides modest ranking gains
...
Dense retrievers~\cite{20} bridge that vocabulary gap, but bring
...
structural boilerplate~\cite{12}, and off-the-shelf embedding models
often degrade on specialised corpora outside their training
distribution~\cite{14,19}.
...
significance testing, and per-query failure analysis; and
...
retrieval.
Section~III describes the methodology: corpus construction, the two
retrievers, and the fusion method.
Section~IV details the benchmark construction protocol, metrics, and
experimental configuration.
Section~V presents the performance evaluation, including significance
testing and per-query failure analysis; Section~VI discusses the
implications; and Section~VII concludes with limitations and future
work.
...
reliable relevance signal and dense models underperform because
semantic similarity glosses over legally binding phrasing.
Zheng et al.~\cite{8} find the opposite failure direction: lexical
models degrade on reasoning-intensive queries, which semantic models
...
Li et al.~\cite{22} present a contrasting case: on large-scale Chinese
legal case retrieval (LeCaRDv2), BM25 consistently outperforms
off-the-shelf dense models, indicating that the balance between the
paradigms is corpus- and language-dependent; our results suggest it
also depends on how the evaluation queries are phrased.
...
SPLADE~\cite{5} takes a different route, learning sparse
representations that close the vocabulary gap inside a single
inverted index; we stay with classical BM25 because it needs no
training data and keeps the lexical signal fully interpretable.
On the fusion method itself, Cormack et al.~\cite{15} established that
rank-based fusion outperforms score-based combination without
requiring score normalisation, while Bruch et al.~\cite{21} show
analytically that RRF is sensitive to its parameters and that
settings do not transfer reliably across collections, a finding our
validation sweep reproduces on a much smaller statutory corpus.
...
for terminology drafting; this is the closest prior work to our
corpus domain, though it does not evaluate first-stage retrieval
quality.
...
\node[box, minimum width=70mm] (cellar)
  {CELLAR SPARQL\\ {\small 21 EuroVoc concepts $\rightarrow$ 72 instruments}};
\node[box, minimum width=70mm, below=of cellar] (formex)
  {Formex XML parsing + validation\\ {\small 1{,}156 article-level provisions}};
\node[box, minimum width=70mm, below=of formex] (chunk)
  {Hierarchical chunking ($>$7k tokens)\\ {\small 1{,}166 retrieval units}};
\node[box, below left=5mm and -27mm of chunk, minimum width=32mm] (bm25)
  {BM25 index\\ {\small $k_1{=}1.5$, $b{=}0.75$}};
\node[box, below right=5mm and -27mm of chunk, minimum width=32mm] (faiss)
  {FAISS IndexFlatIP\\ {\small nomic-embed, 768-d}};
\node[box, below=21mm of chunk, minimum width=46mm] (rrf)
  {Weighted RRF\\ {\small $k{=}20$, $w_d{=}5$, $w_s{=}1$}};
\node[box, below=of rrf, minimum width=46mm] (out)
  {Top-5 statutory provisions};
\node[box, left=7mm of rrf, minimum width=15mm] (query) {Query};
...
\textit{allowances} can appear in differently scoped provisions.
Stopword removal, in turn, would break fixed legal phrases such as
\textit{free of charge} and \textit{not later than}, whose function
words carry the legal meaning.
...
\begin{axis}[
  ...
  xtick={1,2,3,5,8,10},
  ymin=0.65, ymax=0.85,
  legend style={font=\small, at={(0.98,0.02)},
...
tasks across jurisdictions~\cite{23}, it targets a single tightly
...
Gains are binary, and because retrieval is article-level, several
articles of the same instrument can occupy the top~5; each relevant
instrument is therefore credited once, at its highest-ranked
occurrence, since
...
paired, non-parametric test appropriate for skewed per-query scores,
and we report Cohen's~$d$ alongside $p$-values so that effect sizes
are visible even where significance is not reached.

\subsection{Hardware and Software}

All reported results were produced on consumer hardware without GPUs
or paid APIs: the primary systems on an AMD Ryzen~7 5700U laptop
(CPU-only, Windows~11) and the embedding-model ablation on an
Apple~M1~Pro (CPU-only), with the primary-system numbers reproduced
identically on both machines: deliberately modest hardware, since one
claim of this work is that the full pipeline is reproducible
anywhere.
...
Table~\ref{tab:effectiveness} reports the headline comparison on the
sealed 53-query test set.
...
\caption{Retrieval effectiveness on the 53-query test set.
  ...
  Bold marks the best of the three primary systems; the last two rows
  are secondary embedding-model ablations (Section~V-G), not used in
  the hybrid.}
...
Hybrid RRF
  ...
  & \textbf{0.721} \\
\midrule
E5-large-v2 (ablation)
  & 0.925 (0.849--0.981)
  & 0.806
  & 0.794 \\
BGE-small-v1.5 (ablation)
  & 0.943 (0.868--1.000)
  & 0.731
  & 0.696 \\
\bottomrule
...
hybrid posts the best $\MRR{5}$ (0.773 vs.\ 0.742) and $\NDCG{5}$
...
$[0.639, 0.836]$; $\NDCG{5}$: hybrid $[0.627, 0.807]$ vs.\ dense
$[0.621, 0.791]$).

Part of the explanation is architectural rather than statistical.
RRF fuses ranks and discards the scores behind them: a dense rank-1
backed by a large similarity margin and one that barely edged out the
runner-up contribute the same $w_d/(k{+}1)$ to the fused score, so
the fusion cannot weight the dense retriever's confidence.
On a corpus of 1{,}166 units, where dense retrieval already places a
relevant instrument at rank~1 for most queries, BM25's ranks carry
little additional discriminative information, and what they do carry
is as likely to displace a correct dense result as to confirm it
(Section~V-E quantifies both directions).
This is a design limitation of rank-based fusion on small statutory
corpora rather than a defect of the tuning: Bruch et al.~\cite{21}
show analytically that rank-based fusion sacrifices score information
that well-normalised convex score combinations can use.
...
the mean but not enough to reach significance.

\subsection{Embedding-Model Ablation}

The choice of \nomic rested partly on its context window
(Section~III), so we tested how much that choice matters: the
dense-only evaluation was re-run under identical conditions with two
alternative open-weight bi-encoders, E5-large-v2~\cite{24} (335M
parameters, 512-token window) and BGE-small-v1.5~\cite{25} (33M
parameters, an order of magnitude smaller, 512-token window).
Both index the 1{,}156 raw articles without the deployed pipeline's
hierarchical splitting, and both silently truncate the 21.1\,\% of
retrieval units that exceed 512 tokens (Fig.~\ref{fig:tokhist}).
Table~\ref{tab:effectiveness} reports their scores.
Neither model differs significantly from \nomic on any metric: across
the six Wilcoxon comparisons, $p$ ranges from 0.055 to 1.0.
E5-large-v2 posts the best MRR and NDCG point estimates of any system
in the table ($\MRR{5}{=}0.806$ vs.\ 0.742, $p{=}0.216$;
$\NDCG{5}{=}0.794$ vs.\ 0.709, $p{=}0.055$), and it ties \nomic on
$\HR{5}$ while hitting a different query subset (49/53 agreement:
E5 recovers q054 and q083 where \nomic misses, and misses q086 and
q088 that \nomic hits).
BGE-small-v1.5, at a fraction of the parameter count, posts the best
coverage ($\HR{5}{=}0.943$, $p{=}0.564$), retrieving q080, a query
all three primary systems miss, from exactly the same indexed text
(Section~V-E).
Two readings follow, and we keep them apart.
First, context-window length does not predict retrieval quality on
this corpus: models that truncate a fifth of it are statistically
indistinguishable from one that reads every article whole, so the
truncation argument for model selection carries less weight than we
assumed at design time.
Second, these are null results at $n{=}53$: failing to detect a
difference is not evidence of equivalence, and we claim parity only
in that weak sense.
...
re-rank what dense already found (precisely what the per-query
analysis shows), while the dense-favouring weight needed to
...
That includes comparisons in our own earlier drafts.
...
small specialised corpora, consistent with the parameter-sensitivity
analysis of Bruch et al.~\cite{21}.
A post-hoc sweep of the full grid ($k{\in}\{10,20,30,60\}$,
$w_d{\in}[1,20]$, three retrieval budgets) on the sealed test set,
run to characterise sensitivity rather than to re-select parameters,
reproduces the deployed configuration's numbers exactly; at the
deployed $k{=}20$, $\HR{5}$ rises monotonically with $w_d$ to a
plateau at the dense-only ceiling (0.925) from $w_d{=}8$, so the
tuned operating point sits on a broad ridge, not a narrow spike
(Fig.~\ref{fig:heatmap}).
...
from documented European Parliament written questions, and a three-way
...
Between the two, there is no measurable difference; fusion acts as a
cheap safety net rather than a necessary ingredient.
An embedding-model ablation sharpens the picture: two further
bi-encoders with 512-token windows are statistically indistinguishable
from the deployed long-context model, so encoder identity, not
context length, is where the remaining variance lives.
...
embedding-model selection, not corpus preprocessing, is the most
promising next step for practitioners.
...
although the ablation adds two further encoders, all three are
general-domain bi-encoders, so the ``dense is enough'' conclusion
should be read as a claim about strong modern bi-encoders on small
statutory corpora, not about dense retrieval in general.
...
relevance judgements to article level; addressing the
cross-instrument discrimination failure at the encoder level, whether
by fine-tuning or by re-ranking; scaling the test set through the European Parliament's open
...
Generative AI tools (Claude, Anthropic) were used solely for proofreading and improving clarity of writing. All research ideas, technical content, and academic contributions are entirely  our own.
